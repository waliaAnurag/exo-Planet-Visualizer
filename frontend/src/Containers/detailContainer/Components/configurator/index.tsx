...
function TabelConfigurator({ showModal, setShowModal, columnData, ConfigureTable }: IProps) {
    ...
    const updateColData = (col: dropdownOptions) => {
        ...
        setConfiguratorData({...configureTableData, ...configureTable})
        setShouldShowLabel(configureTable.configuredColumns.length <= 4 && configureTable.configuredColumns.length != 0)
    }
            ...
            <div>
                <div className="m-3 font-display text-xl flex justify-between flex-wrap">
                    <div>
                        ...
                        {shouldShowLabel && <label> Please select atleast 5 columns</label>}
                    </div>
                    <div>
                        <h4 className='mb-[5px] font-bold'>{t("DASHBOARD.CONFIGURATOR.STAR_PLANET_CONFIGURE")}</h4>
                        ...
                        <div className='mb-[5px]'>
                            <h4 className='mb-[10px] font-bold'>{t("DASHBOARD.CONFIGURATOR.YEAR_DURATION")}</h4>
                ...
                <div className="w-[100%] flex justify-center mt-[5px]">
                    <Button disabled={Object.keys(configureTableData).length === 0 || shouldShowLabel } buttonStyles=" !w-[80%]" buttonText='Configure Table' buttonFunc={() => {
                        setShowModal(false);
